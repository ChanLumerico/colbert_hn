...
## 6. 최종 랭킹 보정 실험 (Intervention Results)

Ablation Study를 통해 도출된 최적의 **LayerRouter (Interaction + LayerNorm)** 아키텍처를 실제 ColBERT 검색 파이프라인의 Re-ranking 단계에 적용하여, 라우터의 개입이 실질적인 검색 품질(NDCG) 향상으로 이어지는지 검증하였다.

### 6.1 개입 강도($\gamma$)에 따른 전체 성능 변화 (Full Parameter Sweep)
검색 점수 보정식 $S_{new} = S_{colbert} + \gamma \cdot S_{router}$ 에서 라우터의 영향력을 조절하는 파라미터 $\gamma$를 변화시키며 관측한 결과이다.

<p align="center">
  <img src="./figures/fig7_sweep_heatmap.png" alt="Figure 7: Intervention Performance Sweep Heatmap" width="800">
</p>

*   **분석:** 위 히트맵은 베이스라인($\gamma=0$) 대비 NDCG@10의 상대적 향상도(%)를 나타낸다. 모든 데이터셋에서 공통적으로 매우 낮은 $\gamma$ 구간(0.001~0.05)에서 붉은색(상승) 영역이 관측되며, 이는 모델의 제안된 Intervention 방식이 도메인에 관계없이 유효한 개입 지점을 가짐을 시각적으로 입증한다.

### 6.2 결과 분석 및 시사점
1.  **성능 향상의 일관성**: 모든 평가 대상 도메인에서 특정 $\gamma$ 구간 내에 베이스라인을 상회하는 지점(Sweet Spot)이 일관되게 존재함을 확인하였다. 이는 ColBERT의 내부 레이어 신호가 실제 랭킹 오류를 교정하는 데 유효한 근거를 제공함을 입증한다.
2.  **Micro-Intervention 현상**: 최적의 가중치가 0.1 미만의 매우 낮은 수치에서 형성되었다. 이는 라우터의 판별 로짓(Logit)이 검색 점수 체계를 압도할 만큼 강력하므로, 실제 서비스 적용 시에는 매우 미세한 가중치를 통한 '부드러운 개입'이 필수적임을 시사한다.
3.  **데이터셋별 상이한 감도**: `SciFact`는 비교적 큰 개입(0.05)에서, `NFCorpus`는 매우 미세한 개입(0.001)에서 최적 성능을 보였다. 이는 도메인 특성에 따라 라우터 신호의 신뢰도를 다르게 조절해야 함을 의미하며, 향후 쿼리 난이도에 따른 동적 가중치 조절 연구의 필요성을 시사한다.
